import { Button, TwoColsLayout } from "@mercurymd/react"
import { TwoColsLayout as OriginalTwoColsLayout, Left, Right } from "./components/TwoColsLayout"

# Presentation

Hoge

- abc
- fuga

1. `bar`
2. $a=1$

---

# Code

fuga `hoge`

```ts
const hello = () => {
  console.log("Hello, world!");
}
```

```python title="hello.py" showLineNumbers
def hello:
    print("Hello, world!")
```

---

# Code with class

```python class="m-4 border-blue-400 shadow-lg shadow-blue-200 dark:shadow-blue-900" title="hello.py"
def hello:
    print("Hello, world!")
```

---

# MDX (React, TailwindCSS)

<div class="space-y-8">
  <div class="flex flex-row items-center gap-1">
    <span class="text-red-600 underline">You</span>
    <span class="text-green-500 italic hover:bg-red-400 font-serif">can</span>
    <div class="bg-gradient-to-r from-cyan-600 to-blue-500 p-1 rounded-sm text-white animate-bounce">style</div>
    <span class="border p-1 rounded-full hover:bg-muted font-mono">with</span>
    <span class="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 rounded-md p-1 animate-pulse">TailwindCSS</span>
  </div>

  <div class="flex flex-row gap-4 items-center">
    <div>
      You can use React components:
    </div>
    <Button>Click me</Button>
    <Button variant="secondary">Click me</Button>
    <Button variant="outline">Click me</Button>
    <Button variant="ghost">Click me</Button>
    <Button variant="link">Click me</Button>
  </div>

  ```mdx
  import { Button } from "@mercurymd/react"

  <div class="flex flex-row gap-4 items-center">
    <div>
      You can use React components:
    </div>
    <Button>Click me</Button>
    <Button variant="secondary">Click me</Button>
    <Button variant="outline">Click me</Button>
    <Button variant="ghost">Click me</Button>
    <Button variant="link">Click me</Button>
  </div>
  ```
</div>

---

# Piyo

piyo

---

# Foo

foo

- hoge
- fuga

---

# Math

Inline math: $e^{i\pi} + 1 = 0$

$$
\forall \epsilon > 0, \exists \delta > 0 \text{ s.t. } |x - x_0| < \delta \Rightarrow |f(x) - f(x_0)| < \epsilon
$$

---

# Original TwoColsLayout

<OriginalTwoColsLayout>
  <Left>
    ### Left side

    - You can put any content here
    - Including lists, images, code blocks, etc.

    ```python
    def greet(name: str) -> str:
        return f"Hello, {name}!"
    ```
  </Left>
  <Right>
    ### Right side

    1. This is the right column
    2. You can also use **MDX** here

    $$
    \int_a^b f(x) dx = F(b) - F(a)
    $$
  </Right>
</OriginalTwoColsLayout>

---

# Show Line Numbers

```ts showLineNumbers
const greet = (name: string): string => {
  return `Hello, ${name}!`;
};

console.log(greet("World")); // Output: Hello, World!
```

---

# Line Highlighting

```ts {2,5-7}
const greet = (name: string): string => {
  return `Hello, ${name}!`;
};

for (const name of ["World", "Mercury", "MDX"]) {
  console.log(greet(name)); // Output: Hello, <name>!
}
```

```ts
const greet = (name: string): string => {
  return `Hello, ${name}!`; // [!code highlight]
};
```

```ts
// [!code highlight:3]
const greet = (name: string): string => {
  return `Hello, ${name}!`;
};
```

---

# Word Highlighting

```ts /string/
const greet = (name: string): string => {
  return `Hello, ${name}!`;
};
```

```ts
// [!code word:string]
const greet = (name: string): string => {
  return `Hello, ${name}!`;
};
```

---

# Diff

```ts diff
  const greet = (name: string): string => {
-   return `Hello, ${name}!`;
+   return `Hi, ${name}!`;
  };
```

```ts
const greet = (name: string): string => {
  return `Hello, ${name}!`; // [!code --]
  return `Hi, ${name}!`; // [!code ++]
};
```

---

# Combined

```ts title="greet.ts" showLineNumbers
const greet = (name: string): string => {
  return `Hello, ${name}!`;  // [!code highlight]
};

console.log(greet("World")); // Output: Hello, World!
```

---

# Non top-level `---` should be preserved

<div>
  hoge

  ---

  fuga
</div>

---

# TwoColsLayout

<TwoColsLayout>
  ### Left side

  - You can put any content here
  - Including lists, images, code blocks, etc.

  ---

  ### Right side

  1. This is the right column
  2. You can also use **MDX** here
</TwoColsLayout>
