...
`<QRCode>` fills the generated modules with `currentColor`, so you can rely on Tailwind's text
utilities (or your own CSS via the `className` prop) to change the foreground colour. Wrap the component when you need
backgrounds, padding, or a custom quiet zone.

```mdx
<div className="inline-flex rounded-lg bg-blue-50 p-4 text-blue-600">
  <QRCode value="https://mercury-md.dev" />
</div>
```
...
- `errorCorrectionLevel` – sets the error correction level. Accepted values are `"L"`, `"M"`,
  `"Q"`, `"H"` as
